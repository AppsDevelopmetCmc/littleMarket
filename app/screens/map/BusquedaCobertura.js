...
import MapView, { PROVIDER_GOOGLE, Polygon, ProviderPropType } from 'react-native-maps';
...
export class BusquedaCobertura extends Component {
   ...
   obtenerPuntosSectores = async () => {
      ...
      for (let j = 0; j < puntos.length; j++) {
         var coord = { ptsSect: puntos[j].puntos };
         ptos.push(coord);
      }
      // global.coordSect = ptos;
      this.setState({ puntosSectores: ptos })
   }
   ...
   actualizarDireccionPedido = async () => {
      ...
      if (validar) {
         console.log("******DIRECCION STATE******" + this.state.direccion)
                  ...
                  <View style={{ flex: 5 }}>
                     {!this.state.siguienteMapa && (
                        ...
                        >
                           ...
                           {this.state.puntosSectores.map((ptosSector, index) => (
                              <View key={index}>
                                 <Polygon
                                    coordinates={ptosSector.ptsSect}
                                    strokeColor="#F00"
