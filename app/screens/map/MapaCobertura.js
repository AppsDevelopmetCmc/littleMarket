...
import { ItemMapDireccion } from './compnentes/ItemMapDireccion';
...
export class MapaCobertura extends Component {
   constructor(props) {
      ...
      this.direccion = { latitud: -0.12354597141569978, longitud: -78.5312076844275 };
   ...
   obtenerPuntosSectores = async () => {
      ...
      console.log('*******puntos**********',puntos[0].puntos);
    /*  const polygon = puntos[0].puntos.map(coordsArr => { 
         console.log('*********PUNTOS POLYGON**********----'+ JSON.stringify(coordsArr));
         let coords = {
     ...
     console.log('*********POLYGON----'+ JSON.stringify(polygon));*/
      ...
      this.setState({puntosSectores: puntos[0].puntos});
   }
   componentDidMount() {
   ...
   render() {
      ...
      console.log('----RENDER MAPA PUNTOS SECTORES----', this.state.puntosSectores);
      ...
      return (
         <SafeAreaView style={[styles.container]}>
            <View style={{ flex: 4 }}>
               ...
               {this.state.region && this.state.puntosSectores.length>0 ? (
                  <View style={{ flex: 5 }}>
                     {!this.state.siguienteMapa && (
                        ...
                        >
                           {/**<MapView.Marker
                              ...
                              draggable={true}
                           />*/}
                           {console.log("*********ANTES DEL POLYGON*****" +JSON.stringify(this.state.puntosSectores))}
                           <MapView.Polygon
                              coordinates={this.state.puntosSectores}
                              strokeColor="#F00"
